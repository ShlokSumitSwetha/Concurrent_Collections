package Complete_Concurrent_Collections.reference.queue;
...
public class ReferenceQueueTest2<K, V> {
	...
	public void put(K key, V value, Object garbageObject) {
		...
		GarbageReference reference = new GarbageReference(garbageObject, key,
				value, map);
	...
	static class CleanupThread extends Thread {
		...
		public void run() {
			while (true) {
				try {
					GarbageReference ref = (GarbageReference) referenceQueue
							.remove();
					while (true) {
